...
export class WebGL2RenderingContext {
  ...
  texSubImage2D(target, level, xoffset, yoffset, ...rest) {
    // 7-arg: texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels/pboOffset)
    // 5-arg: texSubImage2D(target, level, xoffset, yoffset, format, type, source) — source is Image/Canvas
    // 8-arg: texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels, srcOffset)
    if (rest.length >= 4) {
      ...
      // 5-arg form: (target, level, xoffset, yoffset, format, type, source)
      const [format, type, source] = rest
      if (source && typeof source === 'object' && source.width && source.height) {
        // Extract pixel data from Image or Canvas via a temp 2D canvas
        const w = source.width
        ...
        if (pixels) {
          this._gl.glTexSubImage2D(target, level, xoffset, yoffset, w, h, format, type, pixels)
        }
      }
    }
  }
  ...
  getParameter(pname) {
    ...
    // 64-bit integer queries: glGetIntegerv truncates/clamps these to INT_MAX (2147483647),
    // which downstream consumers (e.g. PPSSPP sizing a uniform push buffer) misuse as a real size
    // → multi-GB allocation / OOB write → heap corruption. Read the true 64-bit value, fall back to
    // the 32-bit query, and clamp the WebGL2 spec-bounded ones to a sane maximum.
    if (pname === GL.MAX_UNIFORM_BLOCK_SIZE || pname === GL.MAX_ELEMENT_INDEX ||
        pname === GL.MAX_SERVER_WAIT_TIMEOUT) {
      const buf64 = new Int32Array(2)
      this._gl.glGetInteger64v(pname, buf64)
      let v = buf64[0] + buf64[1] * 0x100000000
      if (!Number.isFinite(v) || v <= 0) {
        const b32 = new Int32Array(1)
        this._gl.glGetIntegerv(pname, b32)
        v = b32[0] >>> 0
      }
      // Some software GL stacks report INT_MAX here; consumers must not size buffers from that.
      if (pname === GL.MAX_UNIFORM_BLOCK_SIZE && (v > 65536 || v <= 0)) v = 65536
      // MAX_ELEMENT_INDEX is legitimately huge in the spec, but software stacks reporting INT_MAX
      // can make consumers over-allocate; cap at 2^24-1 (16M, ample for any PSP draw).
      if (pname === GL.MAX_ELEMENT_INDEX && (v > 0xFFFFFF || v <= 0)) v = 0xFFFFFF
      return v
    }
